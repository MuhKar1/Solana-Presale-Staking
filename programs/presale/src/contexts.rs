...
pub struct CreateMuhoroToken<'info> {
    #[account(init, payer = admin, mint::token_program = token_program, mint::decimals = decimals, mint::authority = mint_authority)]
...
pub struct InitializePresale<'info> {
    ...
    /// CHECK: PDA signer with distinct seed avoids address collision with token_vault
    #[account(seeds = [VAULT_AUTHORITY_SEED, state.key().as_ref()], bump)]
    pub vault_pda: AccountInfo<'info>,

    /// CHECK: Lamport vault PDA; created with System Program owner for SOL transfers.
